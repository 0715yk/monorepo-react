...
var createFieldValidator = (field, setErrors) => {
  const setFieldError = curry((type, message) => {
  ...
  const createCheckField = (validators) => pipe(
...
var useValidation = ({ formData, metadata }) => {
  ...
  const prevFormData = useRef2(formData);
  const validatorRef = useRef2({});
  ...
  const handleFirstRenderingField = handleSetErrors(void 0);
  const handleValidate = (formData2) => {
    Object.keys(formData2).filter(filterActualChangedFields).forEach(
      alt(
        checkValidatorSaved,
        alt(
          checkValueIsReset,
  ...
  const resetPrevFormData = (formData2) => {
    prevFormData.current = formData2;
  };
  useSkipFirstRender(() => {
  ...
  const initializeErrors = useCallback((formData2) => {
  ...
  const initializeValidator = useCallback(
    (field) => {
      validatorRef.current[field] = createFieldValidator(
        field,
        setErrors
      ).createCheckField(metadata[field].validators);
    },
    [metadata]
  );
  ...
  const isValidData = (message) => message === "";
